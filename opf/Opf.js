define(['lodash', 'event-emitter', './IllegalArgumentException', 'turf'],
function (_, EventEmitter, IllegalArgumentException, turf) {
  const refreshStates = ['ready', 'refreshing', 'timeout', 'disabled'];
  const elemTypes = ['node', 'way', 'relation'];
  const numberTags = ['phone', 'fax', 'contact:phone', 'contact:fax'];
  const maxRefreshBboxSize = 9000000;
  const refreshTimout = 3000;
  ...
  class Opf extends EventEmitter {
    ...
    // Static constants (handy for other components & further expansion)
    static get refreshStates() {
      return refreshStates;
    }

    static get elemTypes() {
      return elemTypes;
    }

    static get numberTags() {
      return numberTags;
    }

    static get maxRefreshBboxSize() {
      return maxRefreshBboxSize;
    }

    static get refreshTimout() {
      return refreshTimout;
    }

    static get overpassInterpreterURL() {
      return overpassInterpreterURL;
    }
    ...
    _setRefreshState(state) {
      ...
      if (!Opf.refreshStates.includes(state))
        throw new IllegalArgumentException('state', `must be one of: ${Opf.refreshStates}`);
    ...
    set bbox(bbox) {
      ...
      console.log(size);
      if (size >= Opf.maxRefreshBboxSize) {
        this._setRefreshState('disabled');
      } else if (size < Opf.maxRefreshBboxSize && this.refreshState === 'disabled') {
    ...
    /**
     * Refreshes the stored elems according to the current bbox
     * iff a refresh is possible, otherwise it gets scheduled.
     *
     * @emits newElems iff new elems are added, callback args: elems
     */
    refresh() {
      ...
      if (refreshState === 'ready') {
        ...
        this._refreshScheduled = false;
        this._fetchOverpassData().then((data) => {
          let elems = this._parseOverpassData(data);
          let newOrChangedElems = this._addNewData(elems);
          updateMetadata(newElems).then(() => {
            ...
            window.setTimeout(this.refreshTimeoutEnd, Opf.refreshTimout);
          })
        });
      } else {
    ...
    _fetchOverpassData(query) {
      return new Promise((resolve, reject) => {
        console.debug('Fetching overpass data...');

        var req = new XMLHttpRequest();
        req.open("POST", Opf.overpassInterpreterURL);
        ...
        req.addEventListener('load', e => {
          try {
            ...
            if (e instanceof SyntaxError)
              return reject(`Failed to parse JSON: ${req.response}`);
            else
              throw e;
          }

          if (json.elements === undefined)
            return reject(`No elements array found: ${req.response}`);

          return resolve(json.elements);
        });
        ...
        req.send(`data=${encodeURIComponent(this._overpassQuery)}`);
      });
    }

    _overpassQuery(e) {
      var bbox = this.bbox; // Creates a copy, can safely mutate
      // lng, lat -> lat, lng
      for (let point of bbox) {
        let lat = point[1];
        point[1] = point[0];
      ...
      var query =
      `[out:json][timeout:25][bbox:${bbox.toString()}];
      (
      ${
        Opf.numberTags.map(numberTag => '' +
          Opf.elemTypes.map(elemType => '' +
            `  ${elemType}["${numberTag}"];`
          ).join('\n')
        ).join('\n')
      }
      ...
      return query.replace(/^ {6}(?!= )/gm, ''); // Removes
    }

    _parseOverpassData(data) {
      var parsedData = [];

      // Loop over all OSM elements in the overpass dataset
      for (let osmData of data) {
    ...
    /**
     * Adds new data to the in-memory database ;).
     *
     * @returns New (non-duplicate) elements added
     */
    function _getChangedData(newData) {
      return newData.reduce((newElem, changes) => {
        let dupElem = opfData.find(testElem =>
          newElem.osmData.type === testElem.osmData.type &&
          newElem.osmData.id   === testElem.osmData.id
        );
        if ()

        if (dupElem === undefined) /* Pos changed */ {
          changes.pos.push(newElem);
          changes.pos.push(newElem);
        } else if (!_.isEqual(dupElem.osmData, newElem.osmData)) /* Changed */ {
          dupElem.osmData = newElem.osmData;
          returnData.changedElems.push(dupElem);
        }
        /* Else unchanged */
      }, {
        pos: [], // Position changed
        numberTag: [] // At least one number tag changed
      })
    }

    /**
     * Adds and formats new data to the in-memory "database" ;).
     *
     * @returns {elem]} Array of new or in any way modified elements
     */
    function _pushFormatNewElems(newElems) {
      for (let newElem of newElems) {
        let dupElem = this._elems.find(testElem =>
          newElem.osmData.type === testElem.osmData.type &&
          newElem.osmData.id   === testElem.osmData.id
        );

        // First update reverse geocoding data (for country code)
        let isNew = (dupElem === undefined);
        if (isNew) {
          changes.pos.push(newElem);
          changes.pos.push(newElem);
        } else if (elem.osmData.lat !== other.osmData.lat
                || elem.osmData.lon !== other.osmData.lon) {
    ...
    updateMetadataSingle(elem) {
      return new Promise((resolve, reject) => {
        var type = elem.osmData.type.slice(0, 1).toUpperCase();
        var id = elem.osmData.id;
    ...
    reverseGeocode(osmType, osmId) {
      return new Promise((resolve, reject) => {
        var ovpOsmType = osmType.slice(0, 1).toUpperCase(); // N, W, R

        var req = new XMLHttpRequest();
        req.open("GET", `https://nominatim.openstreetmap.org/reverse?format=json&osm_type=${ovpOsmType}&osm_id=${osmId}`);
